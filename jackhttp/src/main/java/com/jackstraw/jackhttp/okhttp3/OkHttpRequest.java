...
public class OkHttpRequest {
    ...
    public static final MediaType MEDIA_TYPE_JSON = MediaType.parse("application/json; charset=utf-8");//JSON数据格式
    public static final MediaType MEDIA_TYPE_STREAM = MediaType.parse("application/octet-stream");//二进制流数据
    public static final MediaType MEDIA_TYPE_TEXT = MediaType.parse("text/plain");//纯文本格式
    ...
    private static RequestBody builderFileFormData(File file, String pic_key, List<File> files, Map<String, String> params, final HttpCallback callback) {
        UploadRequest countingRequestBody = null;

        if (file != null) {
            RequestBody requestBody = RequestBody.create(MEDIA_TYPE_STREAM, file);
            countingRequestBody = new UploadRequest(requestBody, new UploadRequest.Listener() {
                @Override
            ...
            countingRequestBody = new UploadRequest(builder.build(), new UploadRequest.Listener() {
                @Override
                public void onRequestProgress(long byteWritted, long contentLength) {
                    OkHttpUtils.sendProgressResultCallback(byteWritted, contentLength, callback);
                }
        ...
        getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                ...
                try {
                    ...
                    OkHttpUtils.sendSuccessResultCallback("下载成功", callback);
                } catch (IOException e) {
                    e.printStackTrace();
                    OkHttpUtils.sendFailResultCallback(-1, e.getMessage(), callback);
                } finally {
